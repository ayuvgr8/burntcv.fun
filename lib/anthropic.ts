...
// Which model a platform-key call will actually ask for, given the current env.
// Exported so surfaces that report config (e.g. /api/health) don't have to
// re-derive the routing rule and drift from it.
export const platformModel = process.env.AI_GATEWAY_API_KEY
  ? GATEWAY_MODEL
  : ROAST_MODEL;
